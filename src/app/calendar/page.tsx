...
export default function Calendar() {
    ...
    function changePage(event: ChangeEvent<HTMLSelectElement>) {
        ...
        if (getPlanetState.localeCompare('Earth')) {
            setSelectedYear(
                earthYears[
                    Number(ttime().toLocale('en-us').format('YYYY')) - 1000
                ]
            );
        ...
        if (getPlanetState.localeCompare('Mars')) {
            setSelectedYear(
                marsYears[marsConvertYear(getTimeCC.epochMillis)[0]]
            );
    ...
    let tupleDates =
        getPlanetState == 'Earth'
            ...
            ? Object.entries(
                  marsCalendar(
                      getTimeCC.epochMillis + 88775244 * 41.25 * calMonthDiff
                  )
              )
            : [1337];
    const tooltipConvertToMarsTime = (date: YMDDate) => {
        if (Number(selectYear.year) > 1873) {
            const solsMonth = marsConvertMonth(
                ttime(
